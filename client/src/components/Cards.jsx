  ...
  return (
    user && (
      <div className='flex items-center gap-3 p-2 sm:p-3 bg-white backdrop-blur-xl border border-gray-200 rounded-2xl shadow-md hover:shadow-lg transition-all duration-300 hover:scale-[1.03]'>
        <div className='"w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-amber-500 to-red-500 rounded-2xl flex items-center justify-center shadow-md'>
          <span className='text-base sm:text-lg font-black text-black'>
            {user.name ? user.name.charAt(0).toUpperCase() : ""}
          </span>
        </div>
        <div>
          <div className='text-xs sm:text-sm font-bold text-slate-800'>{user.name || ""}</div>
          <button
            className='text-amber-600 text-[10px] sm:text-xs font-bold cursor-pointer hover:text-red-600 transition-colors'
  ...
  return (
    ...
    >
      ...
      <div className='absolute top-4 right-4 z-10 flex items-center gap-2 px-3 py-2 bg-gray-50 border border-gray-200 rounded-full shadow-sm'>
        <div className='w-3 h-3 rounded-full flex items-center justify-center bg-gradient-to-r from-emerald-500 to-green-600'>
      ...
      <div className={`p-4 sm:p-6 flex-1 relative overflow-hidden bg-gradient-to-br ${designColor}`}>
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <div className='w-full h-[180px] sm:h-[200px] lg:h-[220px] flex flex-col items-center justify-center rounded-2xl'>
            <Edit size={28} className="text-amber-600" />
          </div>
          ...
          <div className='absolute inset-4 sm:inset-6 bg-gradient-to-t from-white/80 via-white/20 to-transparent flex items-end justify-center p-6 opacity-0 group-hover:opacity-100 transition-all duration-300 rounded-2xl'>
            <div className='flex gap-3'>
              <button
                ...
                className='group/btn w-12 h-12 flex items-center justify-center bg-gradient-to-r from-amber-600 to-emerald-600 rounded-2xl shadow-md hover:scale-110 transition-all duration-300'
                title="Edit"
              >
                <Edit size={18} className='text-black group-hover/btn:scale-110 transition-transform' />
              </button>
              <button
                ...
                title="Delete"
              >
                <Trash2 size={18} className='text-black group-hover/btn:scale-110 transition-transform' />
              </button>
            </div>
          </div>
        )}
      ...
      <div className='bg-gray-50 border-t border-gray-200 p-4 sm:p-6'>
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1">
            <h5 className='text-sm sm:text-base font-bold text-slate-800 truncate mb-2 group-hover:text-emerald-600 transition-colors'>{title}</h5>
        ...
        <div className="relative w-full h-2 bg-gray-200 rounded-full overflow-hidden">
          <div
            className={`h-full bg-gradient-to-r from-emerald-500 to-green-600 rounded-full transition-all duration-700 ease-out relative overflow-hidden`}
